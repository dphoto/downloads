...
class Lifecycle extends Services{
	...
	function Lifecycle(){
		...
		$days = 30;
		$files = $this->db->select( "SELECT * FROM files WHERE file_storage = 'standard' AND user_id = $user_id AND DATEDIFF(CURRENT_TIMESTAMP, file_uploaded) > $days AND file_deleted IS NULL and file_backup = 1 LIMIT $limit" );
